...
Next, we plot the predicted values in a way that they're distibguishable from the actual values. For example, let's change their shape:
...
We can now see how the actual and predicted values compare across our predictor variables. In case you'd forgotten, the coloured points are the actual data, and the white circles are the predicted values. With this in mind, we can see, as expected, that there is less variability in the predicted values that the actual values. It also appears that the sepal width of the setosa species is not being as well accounted for as the other species.

## Logistic Regression

To round this post off, lets extend our approach to logistic regression. It's going to be basically the same workflow, except that we need to extract predicted and residual values for the responses. Here's an example predicting V/S (`vs`), which is 0 or 1, with `hp`:
