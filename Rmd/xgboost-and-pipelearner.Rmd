...
For those of you who are comfortable, here's a bonus example examining learning curves:
...
results <- pipelearner(d, pl_xgboost, cancer ~ ., nrounds = 25) %>% 
  ...
  learn() %>% 
  mutate(
    train = pmap_dbl(list(fit, train, target), accuracy),
    test  = pmap_dbl(list(fit, test,  target), accuracy)
  )

results %>% 
  select(train_p, train, test) %>% 
  gather(data, accuracy, -train_p) %>% 
  ggplot(aes(train_p, accuracy)) +
